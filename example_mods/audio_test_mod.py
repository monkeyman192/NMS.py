# /// script
# dependencies = ["pymhf[gui]>=0.2.3", "nmspy"]
#
# [tool.uv.sources]
# pymhf = { path = "D:/github/windows/pyMHF"}
# nmspy = { path = "D:/github/windows/NMS.py"}
#
# [tool.pymhf]
# exe = "NMS.exe"
# steam_gameid = 275850
...
# log_dir = "."
# log_level = "info"
# window_name_override = "NMS audio logger"
# ///
import ctypes
import logging
...
from pymhf.gui.decorators import BOOLEAN, ENUM, STRING, gui_button
...
class AudioState(ModState):
    event_id: cGcAudioWwiseEvents = cGcAudioWwiseEvents.INVALID_EVENT
    obj_id: int = 0
...
class AudioNames(Mod):
    ...
    @property
    @ENUM("Audio ID:", cGcAudioWwiseEvents)
    def select_audio_id(self):
        return self.state.event_id

    @select_audio_id.setter
    def select_audio_id(self, value: cGcAudioWwiseEvents):
    ...
    def play_sound(self):
        if self.state.event_id and self.state.obj_id and self.audio_manager is not None:
            ...
            audioid.muID = self.state.event_id.value
            self.audio_manager.Play(event=ctypes.byref(audioid), object=self.state.obj_id)
